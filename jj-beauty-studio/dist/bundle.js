...
function initNavbar() {
  ...
  if (window.location.search.includes("_t=1")) {
    ...
    if (overlay) {
      // Clean the query param from the URL bar without reloading
...
function renderProductCard(p, toneIndex, topRank = 0) {
  ...
  <article class="card-product" data-id="${p.id}" data-category="${p.category}" data-name="${p.name.toLowerCase()}">
    <a href="product-details.html?id=${p.id}" class="card-media ${tone}" aria-label="View ${p.name}">
      <span class="badge-cat">${CATEGORY_LABELS[p.category]}</span>
    ...
    <div class="card-body">
      ...
      <div class="card-price-row">
        <span class="card-price">₱${Number(p.price || 0).toFixed(2)}</span>
      </div>
      <div class="card-actions">
        <a class="btn btn-outline" href="product-details.html?id=${p.id}">View Details</a>
        <a class="btn btn-primary order-now" href="${MESSENGER_URL}" target="_blank" rel="noopener">${p.stock === "out" ? "Notify Me" : "Order Now"}</a>
      </div>
    </div>
  </article>`;
}
...
function initProductDetailsPage() {
  ...
  setText("#detail-category", CATEGORY_LABELS[product.category] || product.category);
  setText("#detail-name", product.name);
  setText("#detail-price", `₱${Number(product.price || 0).toFixed(2)}`);
...
function createReceiptModal() {
  ...
  wrap.innerHTML = `
    <div class="modal">
      <div class="modal-head">
        <div>
          <h3>Order Receipt</h3>
          <p>Enter your Messenger name, quantity, and a note.</p>
        </div>
      ...
      <div style="margin-top:6px;">
        ...
        <div style="display:flex; gap:10px; margin-top:12px;">
          ...
          <button class="btn btn-primary" id="rcpt-generate">Generate & Open Messenger</button>
        </div>
        <div style="margin-top:12px; color:var(--ink-soft); font-size:.95rem;">
          ...
          <textarea id="rcpt-preview-text" rows="6" style="width:100%; padding:10px; border-radius:8px; border:1px solid var(--line); font-family:inherit;"> </textarea>
          <div style="display:flex; gap:8px; margin-top:8px;">
            <button class="btn btn-ghost" id="rcpt-copy-btn">Copy Message</button>
            <div style="align-self:center; color:var(--ink-soft); font-size:.9rem;">If automatic paste is blocked, paste this into Messenger.</div>
          </div>
          <p style="margin-top:10px; color:var(--ink-soft); font-size:.9rem;">Generate will copy the message and open Messenger automatically. Paste it into the chat to send.</p>
        </div>
      </div>
    </div>
  `;
  ...
  // Copy button
  setTimeout(() => {
    const copyBtn = document.getElementById('rcpt-copy-btn');
    if (copyBtn) {
      copyBtn.addEventListener('click', async () => {
        const txt = document.getElementById('rcpt-preview-text').value || '';
...
function openReceiptModalForProduct(product) {
  ...
  document.getElementById('rcpt-generate').onclick = function () {
...
async function generateReceipt(product) {
  ...
  // Copy message and open Messenger
  try {
        ...
        return `
          <article class="order-card">
            ...
            <div class="order-card-body">
              <div class="order-line"><strong>${item.name || 'Product'}</strong></div>
              <div class="order-line">Quantity: ${item.quantity || 0}</div>
              <div class="order-line">Unit price: ₱${Number(item.unitPrice || 0).toFixed(2)}</div>
              <div class="order-line">Total: ₱${Number(order.totalPrice || 0).toFixed(2)}</div>
            </div>
          </article>
        `;
      }).join('');
  ...
  overlay.innerHTML = `
    <div class="admin-login-card">
      ...
      <form id="admin-login-form">
        ...
        <div style="display:flex; gap:8px; margin-top:8px;">
          <button type="button" id="admin-back-btn" style="flex:1; background:#f1f1f1; color:#222; border-radius:10px; padding:10px; border:none; cursor:pointer;">Back</button>
          <button type="submit" style="flex:1;">Enter</button>
        </div>
        <div style="display:flex; justify-content:space-between; margin-top:10px; gap:8px;">
          <button type="button" id="forgot-password-btn" style="flex:1; background:#eee; color:#222; border-radius:10px; padding:10px; border:none; cursor:pointer;">Forgot Password</button>
          <button type="button" id="change-password-sidebar-btn" style="flex:1; background:#eee; color:#222; border-radius:10px; padding:10px; border:none; cursor:pointer;">Change Password</button>
        </div>
  ...
  function showPasswordModal(mode) {
      ...
      <div class="admin-login-card">
        ...
        <form id="admin-password-action-form">
          ${mode === 'change' ? `
            ...
            <div style="display:flex; gap:8px; margin-top:8px;">
              <button type="button" id="password-modal-cancel" style="flex:1; background:#f1f1f1; color:#222; border-radius:10px; padding:10px; border:none; cursor:pointer;">Cancel</button>
              <button type="submit" style="flex:1;">Save Password</button>
            </div>
          ` : `
            ...
            <div style="display:flex; gap:8px; margin-top:8px;">
              <button type="button" id="password-modal-cancel" style="flex:1; background:#f1f1f1; color:#222; border-radius:10px; padding:10px; border:none; cursor:pointer;">Close</button>
            </div>
          `}
    ...
    passwordForm.addEventListener('submit', (event) => {
      ...
      if (mode === 'change') {
        ...
        if (currentPassword !== getSavedAdminPassword()) {
